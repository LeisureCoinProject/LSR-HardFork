...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zlsrspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LSR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zlsr send
enum ZerocoinSpendStatus {
    zlsr_SPEND_OKAY = 0,                            // No error
    zlsr_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    zlsr_WALLET_LOCKED = 2,                         // Wallet was locked
    zlsr_COMMIT_FAILED = 3,                         // Commit failed, reset status
    zlsr_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    zlsr_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    zlsr_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    zlsr_TRX_CREATE = 7,                            // Everything related to create the transaction
    zlsr_TRX_CHANGE = 8,                            // Everything related to transaction change
    zlsr_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    zlsr_INVALID_COIN = 10,                         // Selected mint coin is not valid
    zlsr_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    zlsr_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    zlsr_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    zlsr_SPENT_USED_zlsr = 14,                      // Coin has already been spend
    zlsr_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    zlsr_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void zlsrBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreatezlsrOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzlsrAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzlsrTracker> zlsrTracker;
    ...
    void setZWallet(CzlsrWallet* zwallet)
    {
        zwalletMain = zwallet;
        zlsrTracker = std::unique_ptr<CzlsrTracker>(new CzlsrTracker(strWalletFile));
    }

    CzlsrWallet* getZWallet() { return zwalletMain; }
    ...
    void setzlsrAutoBackups(bool fEnabled)
    {
    ...
    /// Get 5000LSR input and keys which can be used for the Masternode
    ...
    /** zlsr reset */
    boost::signals2::signal<void()> NotifyzlsrReset;
