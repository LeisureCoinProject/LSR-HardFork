...
public:
    ...
    bool WritezlsrSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzlsrSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzlsrSeed_deprecated(uint256& seed);
    bool ErasezlsrSeed();
    bool ErasezlsrSeed_deprecated();

    bool WritezlsrCount(const uint32_t& nCount);
    bool ReadzlsrCount(uint32_t& nCount);
