...
#include "zlsr/accumulatorcheckpoints.h"
#include "zlsrchain.h"
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightzlsrThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LSR and zlsr money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in LSR/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in LSR/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, leisurecoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all leisurecoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-LSRstake=<n>", strprintf(_("Enable or disable staking functionality for LSR inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zlsrstake=<n>", strprintf(_("Enable or disable staking functionality for zlsr inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzlsr=<n>", strprintf(_("Enable automatic wallet backups triggered after each zlsr minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zlsr spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zlsrbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zlsr backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeleisurecoinamount=<n>     " + strprintf(_("Keep N LSR anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The LeisureCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //leisurecoin: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //leisurecoin specific: zerocoin and spork DB's
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zlsrSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                    if (pblockindex->GetZerocoinSupply() < zlsrSupplyCheckpoint) {
                        // Trigger reindex due wrapping serials
                        LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zlsrSupplyCheckpoint/COIN);
                        reindexDueWrappedSerials = true;
                    } else if (pblockindex->GetZerocoinSupply() > zlsrSupplyCheckpoint) {
                        // Trigger global zlsr reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateLSRSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zlsrSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                    if (pblockindex->GetZerocoinSupply() != zlsrSupplyCheckpoint)
                        return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zlsrSupplyCheckpoint/COIN));
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzlsrWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zlsrWallet
        uiInterface.InitMessage(_("Syncing zlsr wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnablezlsrBackups = GetBoolArg("-backupzlsr", true);
        pwalletMain->setzlsrAutoBackups(fEnablezlsrBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zlsrTracker->Init();
...
//    }
//
//    nAnonymizeleisurecoinAmount = GetArg("-anonymizeleisurecoinamount", 0);
//    if (nAnonymizeleisurecoinAmount > 999999) nAnonymizeleisurecoinAmount = 999999;
//    if (nAnonymizeleisurecoinAmount < 2) nAnonymizeleisurecoinAmount = 2;
    ...
    LogPrintf("Anonymize LeisureCoin Amount %d\n", nAnonymizeleisurecoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1LSR+1000 == (.1LSR+100)*10
       10LSR+10000 == (1LSR+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightzlsrThread(threadGroup);
    }
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", false)) {
            // Run a thread to precompute any zLSR spends
