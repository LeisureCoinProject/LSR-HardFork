...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zlsrObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zlsrObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zlsrObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zlsrsupply", zlsrObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zlsrsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zlsr denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zlsr denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zlsr denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zlsr denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zlsr denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zlsr denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zlsr denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zlsr denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zlsr denominations\n"
            "  }\n"
            "}\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), zlsr_FAILED_ACCUMULATOR_INITIALIZATION);
