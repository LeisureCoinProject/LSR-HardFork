...
CzlsrWallet::CzlsrWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlsr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzlsrSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezlsrSeed_deprecated()) {
                    LogPrintf("%s: Updated zlsr seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlsr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlsr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzlsrWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezlsrCount(nCountLastUsed);
    else if (!walletdb.ReadzlsrCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzlsrWallet::Lock()
{
    seedMaster = 0;
}

void CzlsrWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzlsrWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozlsr(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzlsrWallet::LoadMintPoolFromDB()
{
...
void CzlsrWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzlsrWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzlsrWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlsrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzlsrWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozlsr(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlsrTracker which also adds to database
    pwalletMain->zlsrTracker->Add(dMint, true);
...
void CzlsrWallet::SeedTozlsr(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzlsrWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzlsrWallet::GenerateDeterministiczlsr(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzlsrWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzlsrWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzlsrWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
