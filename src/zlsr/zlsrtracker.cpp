...
CzlsrTracker::CzlsrTracker(std::string strWalletFile)
{
...
CzlsrTracker::~CzlsrTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzlsrTracker::Init()
{
...
bool CzlsrTracker::Archive(CMintMeta& meta)
{
...
bool CzlsrTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzlsrTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzlsrTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzlsrTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzlsrTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzlsrTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzlsrTracker::GetSerialHashes()
{
...
CAmount CzlsrTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzlsrTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzlsrTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzlsrTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzlsrTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzlsrTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzlsrTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzlsrTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzlsrTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzlsrTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzlsrTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzlsrWallet* zlsrWallet)
{
    bool iszlsrWalletInitialized = (NULL != zlsrWallet);
    CMintMeta meta;
    ...
    if (! iszlsrWalletInitialized)
        zlsrWallet = new CzlsrWallet(strWalletFile);
    meta.isSeedCorrect = zlsrWallet->CheckSeed(dMint);
    if (! iszlsrWalletInitialized)
        delete zlsrWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzlsrTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzlsrTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzlsrTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzlsrTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzlsrTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzlsrTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzlsrWallet* zlsrWallet = new CzlsrWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zlsrWallet);
        }
        delete zlsrWallet;
        LogPrint("zero", "%s: added %d dzlsr from DB\n", __func__, listDeterministicDB.size());
    }
