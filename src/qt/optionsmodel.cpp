...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LSR);
    ...
    if (!settings.contains("nAnonymizeleisurecoinAmount"))
        settings.setValue("nAnonymizeleisurecoinAmount", 1000);

    nAnonymizeleisurecoinAmount = settings.value("nAnonymizeleisurecoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeleisurecoinAmount"))
        SoftSetArg("-anonymizeleisurecoinamount", settings.value("nAnonymizeleisurecoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeleisurecoinAmount:
            return QVariant(nAnonymizeleisurecoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeleisurecoinAmount:
            nAnonymizeleisurecoinAmount = value.toInt();
            settings.setValue("nAnonymizeleisurecoinAmount", nAnonymizeleisurecoinAmount);
            emit anonymizeleisurecoinAmountChanged(nAnonymizeleisurecoinAmount);
            break;
        case CoinControlFeatures:
