...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zlsrTracker->HasMintTx(hash))) {
            //zlsr stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::Stakezlsr;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zlsr;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IszlsrType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::Stakezlsr:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zlsr:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::Stakezlsr || type == TransactionRecord::MNReward) {
