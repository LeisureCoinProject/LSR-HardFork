                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zlsr ought to be enough for anybody." - Bill Gates, 2017
    ui->zlsrpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzlsrSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zlsrpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzlsr_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zlsr is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zlsr, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zlsr...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zlsr in ") +
...
void PrivacyDialog::on_pushButtonSpendzlsr_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zlsr is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zlsr, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zlsr
        sendzlsr();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zlsr
    sendzlsr();
}

void PrivacyDialog::on_pushButtonzlsrControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    zlsrControlDialog* zlsrControl = new zlsrControlDialog(this);
    zlsrControl->setModel(walletModel);
    zlsrControl->exec();
}

void PrivacyDialog::setzlsrControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzlsrSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzlsr()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid leisurecoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zlsrpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zlsr is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LSR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zlsr</b>";
    ...
    // use mints from zlsr selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!zlsrControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zlsrControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zlsr was spent successfully update the addressbook with the label
    ...
    // Clear zlsr selector in case it was used
    zlsrControlDialog::setSelectedMints.clear();
    ui->labelzlsrSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zlsr Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " LSR, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zlsr Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zlsrTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zlsr </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zlsr "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zlsr "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zlsr "));
    ui->labelzlsrAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zlsr </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zlsr </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zlsr </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzlsrSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzlsr->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zlsr
        ui->pushButtonMintzlsr->setEnabled(false);
        ui->pushButtonMintzlsr->setToolTip(tr("zlsr is currently disabled due to maintenance."));

        // Spend zlsr
        ui->pushButtonSpendzlsr->setEnabled(false);
        ui->pushButtonSpendzlsr->setToolTip(tr("zlsr is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zlsr
        ui->pushButtonMintzlsr->setEnabled(true);
        ui->pushButtonMintzlsr->setToolTip(tr("PrivacyDialog", "Enter an amount of LSR to convert to zlsr", 0));

        // Spend zlsr
        ui->pushButtonSpendzlsr->setEnabled(true);
        ui->pushButtonSpendzlsr->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
