...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 LSR.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzlsrReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzlsrReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzlsrReset.connect(boost::bind(NotifyzlsrReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzlsrReset.disconnect(boost::bind(NotifyzlsrReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zlsrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
