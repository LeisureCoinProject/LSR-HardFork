...
#include "ui_zlsrcontroldialog.h"
...
std::set<std::string> zlsrControlDialog::setSelectedMints;
std::set<CMintMeta> zlsrControlDialog::setMints;

bool CzlsrControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zlsrControlDialog::COLUMN_DENOMINATION || column == zlsrControlDialog::COLUMN_VERSION || column == zlsrControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zlsrControlDialog::zlsrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zlsrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zlsrControlDialog::~zlsrControlDialog()
{
    delete ui;
}

void zlsrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zlsrControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzlsrControlWidgetItem* itemDenom(new CzlsrControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzlsrControlWidgetItem *itemMint = new CzlsrControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zlsrTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zlsrTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zlsr.";
            else if (!mint.isSeedCorrect)
                strReason = "The zlsr seed used to mint this zlsr is not the same as currently hold in the wallet";
            else
...
void zlsrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zlsrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzlsr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzlsrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zlsrControlDialog::GetSelectedMints()
{
...
void zlsrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
