...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzlsrStake(spend));
    } else {
        ...
        CLSRStake* LSRInput = new CLSRStake();
        LSRInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(LSRInput);
    }
