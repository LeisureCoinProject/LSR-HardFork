...
git clone https://github.com/Liquid369/leisurecoin
cd leisurecoin/src
cd ..
prepare_system
create_swap
...
make install
if [ -d '/root/.leisurecoin' ] ; then
#Things to do
cd /root/.leisurecoin
rm -rf *
touch leisurecoin.conf
echo "rpcuser=user" >> leisurecoin.conf
echo "rpcpassword=pass123" >> leisurecoin.conf
echo "server=1" >> leisurecoin.conf
echo "daemon=1" >> leisurecoin.conf
echo "listen=1" >> leisurecoin.conf
echo "rpcallowip=127.0.0.1" >> leisurecoin.conf
echo "Finished setting up config,now starting daemon"
leisurecoind
echo $endmessage
else
mkdir /root/.leisurecoin
cd /root/.leisurecoin
touch leisurecoin.conf
echo "rpcuser=user" >> leisurecoin.conf
echo "rpcpassword=pass123" >> leisurecoin.conf
